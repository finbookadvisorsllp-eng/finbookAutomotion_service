...
import { Save, Send, ArrowLeft, Plus, Settings, ChevronDown, CheckCircle2, ChevronUp, Trash2, Calendar, Layout, Search, FileText } from 'lucide-react';
import VoucherEntryEngine from './VoucherEntryEngine';

const PurchaseOrder = ({ isDark, onBack }) => {
  const theme = {
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b shrink-0 bg-white/50 backdrop-blur-sm" style={{ borderColor: theme.border }}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-[14px] font-black tracking-tight" style={{ color: theme.text }}>Purchase Order</h1>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-white text-[11px] font-bold hover:bg-emerald-700 transition-colors bg-emerald-600 shadow-sm">
            <CheckCircle2 size={12} /> Create Order
          </button>
        </div>
      </div>
