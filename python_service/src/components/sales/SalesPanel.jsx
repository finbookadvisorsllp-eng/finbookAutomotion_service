...
import {
  ...
  FileSpreadsheet, ScanLine, BarChart3, CloudUpload, Link, Paperclip
} from 'lucide-react';
...
const SalesPanel = ({ mode, isDark, onAdd, title: customTitle, description: customDescription, voucherType = "sales", emptyText, icon: CustomIcon }) => {
  ...
  const handlePushToReviewRow = async (id) => {
    ...
    if (res.success) {
      toast.success('Transaction pushed to review');
      fetchTransactions();
    } else {
  ...
  const handleApproveSelected = async () => {
    ...
    if (window.confirm(`Are you sure you want to approve ${selectedIds.length} selected transaction(s)?`)) {
      ...
      for (const id of selectedIds) {
        const res = await approveTransaction(id, 'Approved from panel bulk action');
  ...
  const ActionButton = ({ onClick, icon: Icon, color, tooltip }) => {
    const colorMap = {
      ...
      purple: 'text-slate-400 hover:text-purple-600 hover:bg-purple-50/50'
    };
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 shrink-0">
            ...
            <div className="relative min-w-[220px] sm:max-w-xs group">
              ...
              <input
                type="text"
                placeholder="Search party, invoice number..."
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              {mode === 'Inbox' && (
                <>
                  ...
                  <IconButton icon={Plus} color="indigo" label="Create Entry" isPrimary onClick={handleCreateNew} />
              ...
              {mode === 'Archive' && (
                <>
                  <IconButton icon={Download} color="purple" label="Export Selected" onClick={handleExportTallySelected} />
                  <IconButton icon={Trash2} color="red" onClick={handleDeleteSelected} />
                  <IconButton icon={RefreshCw} color="emerald" onClick={fetchTransactions} />
                </>
              )}
          ...
          <table className="w-full text-left border-collapse min-w-[1100px]">
            <thead className="sticky top-0 z-10">
              <tr style={{ backgroundColor: isDark ? 'var(--app-table-head-bg)' : '#fcfdfe' }}>
                ...
                <TableHead label="Action" center width="110px" />
              </tr>
            </thead>
            <tbody>
              {transactions.length > 0 ? (
                transactions.map((tx, idx) => (
                  <tr key={tx._id} className="hover:bg-[var(--app-control-hover)] transition-colors text-[10px] border-b" style={{ borderColor: 'var(--app-row-border)', color: 'var(--app-text)' }}>
                    ...
                    <td className="p-1.5 px-2 border-r font-bold text-indigo-600 truncate">
                      {tx.ocrMetadata?.documentUrl ? (
                        <a href={tx.ocrMetadata.documentUrl} target="_blank" rel="noopener noreferrer" className="hover:underline flex items-center gap-1">
                    ...
                    <td className="p-1.5 px-2 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        {tx.status === 'approved' && (
                          <ActionButton
                            onClick={() => handleExportTally(tx._id)}
                            icon={Download}
                            color="purple"
                            tooltip="Download Tally XML"
                          />
                        )}
                        {tx.attachedDocuments?.length > 0 && (
                          <ActionButton
                            onClick={() => window.open(tx.attachedDocuments[0].cloudinaryUrl, '_blank')}
                            icon={Paperclip}
                            color="blue"
                ...
                <tr>
                  <td colSpan={40} className="p-20 text-center">
                    <div className="flex flex-col items-center justify-center">
                      <p className="text-[10px] uppercase tracking-widest text-slate-400" style={{ color: 'var(--app-muted)' }}>
