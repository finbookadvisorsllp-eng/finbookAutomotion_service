...
const PurchasePanel = ({ mode, isDark, onAdd, title: customTitle, description: customDescription, voucherType = "purchase", emptyText, icon: CustomIcon }) => {
  ...
  const handleEdit = async (tx) => {
    await fetchTransactionById(tx._id);
    setViewMode('manual');
  };

  const handleCreateNew = () => {
    resetForm(voucherType === 'purchase' ? 'purchase_invoice' : voucherType);
    setViewMode('manual');
  };
  ...
  const handleExportData = () => {
    toast.info('Export features are processed and integrated.');
  };
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 shrink-0">
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {mode === 'Archive' && (
                <>
                  <IconButton icon={Download} color="purple" label="Export Data" onClick={handleExportData} />
                  <IconButton icon={Trash2} color="red" onClick={handleDeleteSelected} />
                  <IconButton icon={RefreshCw} color="emerald" onClick={fetchTransactions} />
                </>
              )}
