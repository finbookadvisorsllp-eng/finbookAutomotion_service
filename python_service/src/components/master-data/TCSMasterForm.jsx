...
export default function TCSMasterForm({ initialData = null, isEdit = false, onSave, onClose }) {
  const [formData, setFormData] = useState({
    tcsName: '',
    sectionCode: '206C(1H)',
    buyerTypes: ['Company Resident', 'Resident Buyer'],
    applicableRate: 0.1,
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        tcsName: initialData.tcsName || initialData.name || '',
        sectionCode: initialData.sectionCode || initialData.section || '206C(1H)',
        buyerTypes: Array.isArray(initialData.buyerTypes)
          ? initialData.buyerTypes
          : (initialData.buyerTypes ? [initialData.buyerTypes] : ['Company Resident', 'Resident Buyer']),
        applicableRate: initialData.applicableRate ?? initialData.rate ?? 0.1,
        thresholdLimit: initialData.thresholdLimit ?? initialData.threshold ?? 5000000,
  ...
  const toggleBuyerType = (type) => {
    setFormData(prev => {
      const exists = prev.buyerTypes.includes(type);
      const next = exists ? prev.buyerTypes.filter(t => t !== type) : [...prev.buyerTypes, type];
      return { ...prev, buyerTypes: next.length > 0 ? next : [type] };
    });
  };

  const handleSubmit = async (e) => {
    e?.preventDefault();
    if (!formData.tcsName.trim()) {
      toast.error('TCS Name / Nature is required');
      return;
    }
    ...
    try {
      ...
      const payload = {
        tcsName: formData.tcsName.trim(),
        name: formData.tcsName.trim(),
        sectionCode: formData.sectionCode.trim(),
        section: formData.sectionCode.trim(),
        buyerTypes: formData.buyerTypes,
      ...
      if (isEdit && (initialData?._id || initialData?.id)) {
        ...
        const collection = initialData.sourceCollection || 'tcs_entry';
        await apiClient.put(`/masters/generic/${collection}/${id}`, payload, { headers });
        toast.success(`TCS Master "${formData.tcsName}" updated successfully`);
      } else {
        await apiClient.post('/masters/tcs', payload, { headers });
        toast.success(`TCS Master "${formData.tcsName}" created successfully`);
      }

      if (onSave) onSave(payload);
      onClose();
    } catch (err) {
      console.error('Error saving TCS Master:', err);
      toast.error(err.response?.data?.detail || 'Failed to save TCS Master');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="flex flex-col h-full bg-[var(--app-bg)] text-[var(--app-text)] overflow-hidden font-sans animate-in fade-in duration-300">
      ...
      <div className="shrink-0 px-6 py-4 border-b border-[var(--app-border)] bg-[var(--app-panel-bg)] flex items-center justify-between shadow-2xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-extrabold tracking-tight text-[var(--app-heading)]">
              {isEdit ? `Edit TCS: ${formData.tcsName}` : 'Create TCS Master'}
            </h1>
            <p className="text-xs text-[var(--app-muted)]">
              Configure TCS section, collection rate, threshold, and buyer types
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>{saving ? 'Saving...' : isEdit ? 'Update TCS Master' : 'Save TCS Master'}</span>
          </button>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          {/* Card 1: TCS Nature & Section Code */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[var(--app-muted)] mb-1.5">
                  TCS Name <span className="text-rose-500">*</span>
                </label>
                <input
                  type="text"
                  required
                  placeholder="e.g. Sale of Any Goods, Scrap Sale, Timber Produce"
                  value={formData.tcsName}
                  onChange={(e) => updateField('tcsName', e.target.value)}
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. 206C(1H), 206C(1), 206C(1F)"
          ...
          <div className="rounded-2xl border border-[var(--app-border)] bg-[var(--app-panel-bg)] p-6 space-y-4 shadow-xs">
            ...
            <p className="text-xs text-[var(--app-muted)]">
              Select buyer categories eligible for this TCS Nature. These will auto-filter when creating or editing a Ledger.
            </p>

            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 pt-1">
              {BUYER_TYPE_OPTIONS.map(type => {
                const isSelected = formData.buyerTypes.includes(type);
                return (
                  <button
                    key={type}
                    type="button"
