...
export default function CreditLimit() {
  ...
  const columns = [
    ...
    { key: 'used', label: 'Credit Used', align: 'right', sortable: true, render: (v, r) => (
      <span className={r.exceeded > 0 ? 'text-red-500 font-bold' : ''}>{formatINR(v)}</span>
    )},
    { key: 'available', label: 'Available', align: 'right', sortable: true, render: v => v >= 0 ? <span className="text-emerald-600 font-semibold">{formatINR(v)}</span> : '-' },
    { key: 'exceeded', label: 'Exceeded By', align: 'right', sortable: true, render: v => v > 0 ? <span className="text-red-600 font-black">{formatINR(v)}</span> : '-' },
    { key: 'risk', label: 'Status', render: v => (
      <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${
        v === 'High Risk' ? 'bg-red-100 text-red-700' :
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-red-50 border border-red-200 rounded-2xl p-5">
          <p className="text-xs font-bold text-red-600 uppercase tracking-wider mb-1">Total Exceeded Amount</p>
          <p className="text-2xl font-black text-red-700">{formatINR(limitData.reduce((acc, c) => acc + c.exceeded, 0))}</p>
          <p className="text-sm font-semibold text-red-600 mt-1">{limitData.filter(c => c.exceeded > 0).length} customers exceeded limit</p>
        </div>
      </div>
