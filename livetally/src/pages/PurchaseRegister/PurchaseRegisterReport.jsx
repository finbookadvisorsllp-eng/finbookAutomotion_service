...
import { ShoppingCart, ChevronDown, ClipboardList, FileX, Package } from 'lucide-react';

import { useDateRange } from '../../context/DateContext';
import { getPurchaseRegisterDrilldown, exportPurchaseRegister } from '../../api';
...
import GroupTable from '../SalesRegister/views/GroupTable';
import InvoiceTable from '../SalesRegister/views/InvoiceTable';
...
// Purchase Register column chooser). Every option is served by one engine.
const PRIMARY = [
...
// One engine, four purchase-side document registers. `accounting` documents
// (Purchase, Debit Note) post to ledgers and expose the Gross/Net toggle;
// order/receipt documents are non-accounting (value lives in inventory) so they
// show a single value measure and lead with the invoice/quantity list.
const REPORT_CONFIGS = {
  purchase: { title: 'Purchase Register', icon: ShoppingCart, accounting: true, totalLabel: 'Total Purchase', defaultGroupBy: 'month' },
  purchase_order: { title: 'Purchase Order', icon: ClipboardList, accounting: false, totalLabel: 'Total Order Value', defaultGroupBy: 'bill' },
  debit_note: { title: 'Debit Note', icon: FileX, accounting: true, totalLabel: 'Total Debit Notes', defaultGroupBy: 'month' },
  receipt_note: { title: 'Receipt Note', icon: Package, accounting: false, totalLabel: 'Total Value', defaultGroupBy: 'bill' },
};
...
export default function PurchaseRegisterReport({ report = 'purchase' }) {
  const cfg = REPORT_CONFIGS[report] || REPORT_CONFIGS.purchase;
  const { fy, years, selectFy } = useDateRange();
  const [searchParams, setSearchParams] = useSearchParams();

  // ── URL state (single source of truth) ──
  const groupBy = searchParams.get('groupBy') || cfg.defaultGroupBy;
  // Non-accounting documents (orders / receipt notes) have no tax split, so the
  ...
  const refetch = useCallback(() => {
    ...
    getPurchaseRegisterDrilldown(apiParams)
      .then((res) => { if (alive) setState({ loading: false, error: null, data: res?.data ?? null, pagination: res?.pagination ?? null }); })
      .catch((e) => { if (alive) setState({ loading: false, error: e?.message || 'Failed to load Purchase Register', data: null, pagination: null }); });
    return () => { alive = false; };
  }, [apiParams, fy]);
  ...
  const handleExport = (format) => exportPurchaseRegister({
  ...
  // ── Date picker (doubles as FY switcher) ──
  ...
  // ── The "Total Purchase" figure shown on the header (reconciles with the table) ──
  ...
  return (
    <div className="animate-fade-in flex flex-col pb-10 space-y-3">
      {/* Header: title + Total Purchase · toggles · date · export */}
      ...
      {showFilterBar && (
        <FilterBar
          ...
          searchPlaceholder={
            groupBy === 'bill' || level === 1 ? 'Search invoice / vendor…' :
      ...
      {state.loading ? (
        ...
        <>
          {level === 0 && (groupBy === 'bill'
            ? <InvoiceTable data={{ rows: state.data.rows, totals: { total: state.data.total } }} measure={measure} showQty={!cfg.accounting} onDrill={drillVoucher} partyLabel="Vendor" />
            : <GroupTable data={state.data} onDrill={drillGroup} totalLabel={cfg.totalLabel} />)}
          {level === 1 && <InvoiceTable data={state.data} measure={measure} showQty={!cfg.accounting} onDrill={drillVoucher} partyLabel="Vendor" />}
    ...
    <div className="inline-flex rounded-lg p-0.5" style={{ border: '1px solid var(--report-border)' }}>
      {options.map((o) => (
        <button key={o.value} onClick={() => onChange(o.value)}
          className={`px-3.5 h-8 text-[12px] font-bold rounded-md transition-colors ${value === o.value ? 'bg-purple-600 text-white shadow-sm'
    ...
    <div className="inline-flex items-center gap-2">
      ...
      <div className="relative" ref={ref}>
        <button onClick={() => setOpen((o) => !o)}
          className={`flex items-center gap-1.5 px-3 h-9 rounded-lg text-[12px] font-bold transition-colors ${moreActive ? 'bg-purple-600 text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700'}`}
          ...
          <div className="absolute left-0 mt-1 w-44 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl z-50 overflow-hidden animate-fade-in">
            {MORE.map((o) => (
              <button key={o.value} onClick={() => { onChange(o.value); setOpen(false); }}
                className={`w-full text-left px-3 py-2 text-[12px] font-semibold transition-colors ${value === o.value ? 'bg-purple-50 dark:bg-purple-500/15 text-purple-600 dark:text-purple-400'
