...
export default function Breadcrumbs({ ledgerName, currentYearData }) {
  ...
  return (
    <div className="flex items-center gap-3 mb-4 bg-white dark:bg-slate-800 p-2 px-3 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 w-fit md:w-auto md:justify-between">
      <div className="flex items-center gap-3">
        <button 
        onClick={() => navigateTo(voucher ? 'ledger' : 'root')}
        className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors text-slate-800 dark:text-white"
      >
      ...
      <h1 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
        ...
        <button className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors flex items-center justify-center">
