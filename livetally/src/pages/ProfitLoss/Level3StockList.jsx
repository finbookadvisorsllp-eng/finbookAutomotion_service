...
export default function Level3StockList({ ledgerData, currentYearData }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 animate-fade-in overflow-hidden mt-4">
      
      {/* Header with Date Range matching the screenshot (since it moved from ProfitLoss header) */}
      <div className="flex items-center justify-between p-3 px-4 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-slate-50/50 dark:bg-slate-900/30 border-b border-slate-200 dark:border-slate-700 text-[11px] font-bold text-slate-700 dark:text-slate-300">
            ...
            {stockItems.length === 0 ? (
              ...
              stockItems.map(item => (
                ...
                >
                  ...
                  <td className="py-2 px-4 text-[12px] font-medium text-slate-700 dark:text-slate-300 text-right">
                    {item.qty}
                  </td>
                  <td className="py-2 px-4 text-[12px] font-medium text-slate-700 dark:text-slate-300 text-right">
                    {item.rate}
                  </td>
                  <td className="py-2 px-4 text-[12px] font-medium text-slate-800 dark:text-slate-200 text-right">
      ...
      {/* Pagination Footer */}
      <div className="py-2.5 px-4 border-t border-slate-100 dark:border-slate-700/50 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/30">
