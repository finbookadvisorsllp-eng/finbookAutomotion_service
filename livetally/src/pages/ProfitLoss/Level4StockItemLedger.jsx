...
export default function Level4StockItemLedger({ itemId, currentYearData, itemData: propItemData }) {
 ...
 <span className="text-slate-900 font-bold ">₹{itemData.amount}</span>
 </div>
 <div className="font-medium">
 <span className="text-slate-600">HSN code : </span>
 <span className="text-slate-900 font-bold ">{itemData.hsn}</span>
 </div>
 <div className="font-medium">
 <span className="text-slate-600">Closing stock : </span>
 <span className="text-slate-900 font-bold ">{itemData.closingStock}</span>
 </div>
 <div className="font-medium">
 <span className="text-slate-600">Avg Pur Rate : </span>
 <span className="text-slate-900 font-bold ">₹{itemData.avgPurRate}</span>
 </div>
 <div className="font-medium">
 <span className="text-slate-600">GST Rate : </span>
 <span className="text-slate-900 font-bold ">{itemData.gstRate}</span>
 </div>
 </div>
 </div>
 ...
 <div className="flex bg-slate-100 border border-slate-300 rounded-lg shadow-sm text-[12px] font-semibold text-slate-700 p-0.5">
 <button 
 onClick={() => setActiveTab('Summary')}
 className={`px-3 py-1 rounded-md transition-colors ${activeTab === 'Summary' ? 'bg-slate-50 shadow-sm border border-slate-300 text-slate-900 ' : 'hover:bg-slate-200 border border-transparent'}`}
 >Summary</button>
 <button 
 onClick={() => setActiveTab('Customers')}
 className={`px-3 py-1 rounded-md transition-colors ${activeTab === 'Customers' ? 'bg-slate-50 shadow-sm border border-slate-300 text-slate-900 ' : 'hover:bg-slate-200 border border-transparent'}`}
 >Customers</button>
 <button 
 onClick={() => setActiveTab('Suppliers')}
 className={`px-3 py-1 rounded-md transition-colors ${activeTab === 'Suppliers' ? 'bg-slate-50 shadow-sm border border-slate-300 text-slate-900 ' : 'hover:bg-slate-200 border border-transparent'}`}
 >Suppliers</button>
 <button className="px-3 py-1 hover:bg-slate-200 rounded-md transition-colors flex items-center gap-1 border border-transparent">
 ...
 {/* Date Selector */}
 <div className="flex items-center bg-slate-50 border border-slate-300 rounded-lg shadow-sm h-8 ml-2">
 <button className="px-2 text-slate-600 hover:bg-slate-100 transition-colors border-r border-slate-300 h-full flex items-center">
 ...
 <button className="px-2 text-slate-600 hover:bg-slate-50 transition-colors border-l border-slate-300 h-full flex items-center">
 ...
 type="text" 
 placeholder="Search" 
 className="px-3 py-1.5 border border-slate-300 rounded-lg text-[12px] w-48 focus:outline-none focus:ring-1 focus:ring-blue-500"
 />
 <div className="flex items-center gap-2">
 ...
 <td className="py-2.5 px-4 text-[12px] font-medium text-slate-700 text-center">
 {s.lastPurchase}
 </td>
 <td className="py-2.5 px-4 text-[12px] font-medium text-slate-800 text-right">
 {s.totalQty}
 ...
 <tr>
 <td colSpan="4" className="py-6 text-center text-[12px] font-medium text-slate-500 ">
 No data available
 </td>
 </tr>
 ) : null}
