"""Dynamic, Tally-matched Purchase Register engine.
...
company ids, ledger names, GST rates, voucher types or purchase totals.

    Level 0  Register        — rows grouped by the chosen dimension (+ Total Purchase)
...
Purchase classification is universal: a voucher is a purchase when its Tally parent
class ``voucherTypeOrigName == "Purchase"`` (identical across all companies),
...
# Every purchase-side document register is the same engine over a different Tally
# parent class. ``parents`` lists the reserved ``voucherTypeOrigName`` values that
# identify the document. ``accounting`` = whether the voucher posts to ledgers
# (Purchase / Debit Note) vs a non-accounting order/receipt document whose value
# lives only in its inventory lines.
REPORTS = {
    "purchase":       {"parents": ["Purchase"], "accounting": True,  "title": "Purchase Register"},
    "purchase_order": {"parents": ["Purchase Order"], "accounting": False, "title": "Purchase Order"},
    "debit_note":     {"parents": ["Debit Note"], "accounting": True,  "title": "Debit Note"},
    "receipt_note":   {"parents": ["Receipt Note"], "accounting": False, "title": "Receipt Note"},
}


def report_config(report: str | None) -> dict:
    return REPORTS.get(report or "purchase", REPORTS["purchase"])
...
def _register(db, fy, params, start, end, measure, parents, report):
    ...
    # always reconciles with what's on screen.
...
def drilldown(db, fy: str, level: int, params: dict) -> dict:
    ...
    ``params['report']`` selects the document class (purchase / purchase_order /
    debit_note / receipt_note); defaults to purchase.
    """
    start, end = _period(db, fy, params)
    measure = _measure(params)
    cfg = report_config(params.get("report"))
    parents = cfg["parents"]
    report = params.get("report") or "purchase"
