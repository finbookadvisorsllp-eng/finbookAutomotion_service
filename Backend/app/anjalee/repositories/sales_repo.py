...
from app.anjalee.constants.business_constants import (
    SALES_COLLECTION, VOUCHERS_COLLECTION, COUNTERS_COLLECTION
)
...
class SalesRepository(BaseRepository):
    def get_summary_cursor(self, query: Optional[Dict[str, Any]] = None) -> Any:
        q = query or {}
        return self.db[SALES_COLLECTION].find(q, {"grandTotal": 1, "status": 1})

    def count_transactions(self, query: Dict[str, Any]) -> int:
        return self.db[SALES_COLLECTION].count_documents(query)

    def find_transactions(self, query: Dict[str, Any], skip: int, limit: int) -> List[Dict[str, Any]]:
        return list(
            self.db[SALES_COLLECTION]
            .find(query)
            .sort("createdAt", -1)
            .skip(skip)
            .limit(limit)
    ...
    def insert_transaction(self, doc_data: Dict[str, Any]) -> str:
        result = self.db[SALES_COLLECTION].insert_one(doc_data)
        return str(result.inserted_id)

    def find_transaction_by_id(self, tx_id: str) -> Optional[Dict[str, Any]]:
        try:
            return self.db[SALES_COLLECTION].find_one({"_id": ObjectId(tx_id)})
        except Exception:
            return None
    ...
    def delete_transaction(self, tx_id: str) -> bool:
        try:
            result = self.db[SALES_COLLECTION].delete_one({"_id": ObjectId(tx_id)})
